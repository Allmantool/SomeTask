...
namespace Test.Implement {
    public class CustomConsole : IConsole {
        ...
        private string tips = $"{Environment.NewLine}Tip: {Environment.NewLine}" +
                              ...
                              $"5) 'simplesearchall where *.exe' - (alternative variant of 'searchall' with hierarchy represantation{Environment.NewLine}" +
                              $"6) 'up' - moves up to one level in directory tree (return current location){Environment.NewLine}" +
                              $"7) 'count' - returns all subfolders count{Environment.NewLine}" +
                              $"8) 'exit' - exit{Environment.NewLine}";
